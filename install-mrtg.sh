#!/bin/bash
#
# MRTG Professional Monitoring Suite - Enterprise Edition v2.1.8
...
readonly SCRIPT_VERSION="v2.1.8"
...
create_directories() {
    ...
    if [[ "${DRY_RUN}" != true ]]; then
        ...
        # Check if web user exists before attempting ownership change
        if id "${WEB_USER}" >/dev/null 2>&1; then
            log "INFO" "Setting ownership to ${WEB_USER}:${WEB_GROUP}"

            # Try to set ownership, but don't fail if it doesn't work
            if ! chown -R "${WEB_USER}:${WEB_GROUP}" "${MRTG_BASE}" 2>/dev/null; then
                log "WARNING" "Could not set ownership to ${WEB_USER}. Continuing with root ownership."
            fi

            # Also try to set ownership on web directory
            ...
            log "WARNING" "User ${WEB_USER} not found, directories will be owned by root"
        fi
    fi
...
#!/bin/bash
# Rspamd Statistics Collector for MRTG v2.1.8
...
#!/bin/bash
# MRTG Runner - Generated by MRTG Professional Suite v2.1.8
