#!/bin/bash
#
# MRTG Professional Monitoring Suite - Enterprise Edition v2.2.5
...
readonly SCRIPT_VERSION="v2.2.5"
...
install_dependencies() {
    ...
    case "${OS_ID}" in
        ubuntu|debian)
            apt-get update -qq
            # Note: python3-json is not needed; json is part of python3 standard library.
...
EOF
    ...
    if systemctl list-units --full -all 2>/dev/null | grep -q 'snmpd.service'; then
        ...
        systemctl restart snmpd || {
            log "ERROR" "snmpd failed to restart. Checking journal:"
            journalctl -u snmpd --no-pager | tail -n 20 || true
            return 1
        }
...
# with error handling for setfacl (line 1422 fix)
# =============================================================================
create_directories() {
    ...
    if [[ "${PANEL_TYPE}" == "directadmin" ]] && id diradmin >/dev/null 2>&1; then
        ...
        # Set ACLs for DirectAdmin to read the files (non-fatal if fails) - FIXED LINE 1422
        if command -v setfacl >/dev/null 2>&1; then
            ...
            log "DEBUG" "setfacl command not found, skipping ACL setup"
        fi
...
#!/bin/bash
# Rspamd Statistics Collector for MRTG v2.2.5
...
#!/bin/bash
# MRTG Runner - Generated by MRTG Professional Suite v2.2.5
...
EOF
        ...
        if [[ "${PANEL_TYPE}" == "directadmin" ]]; then
            ...
            if id www-data >/dev/null 2>&1; then
                ...
                chown -R apache:apache "${WEB_MRTG_DIR}" 2>/dev/null || true
            fi
        fi
...
<body>
    ...
    <div class="content">
        ...
        <p>MRTG is collecting data. Please wait 5-10 minutes for graphs to appear.</p>
        <p>Server: $(hostname)</p>
        <p>Date: $(date)</p>
    </div>
</body>
...
EOF
        chmod 644 "${WEB_MRTG_DIR}/index.html" 2>/dev/null || true
    fi
...
verify_system_health() {
    ...
    if command -v snmpget >/dev/null 2>&1; then
        ...
        if [[ "${snmp_ok}" == true ]]; then
            ...
            echo -e "  ${YELLOW}⚠${NC} SNMP not responding - check community string: ${SNMP_COMMUNITY}"
...
This installer will:
  ...
  ✓ Configure Apache for proper web access
  ✓ NEVER modify your web server core configuration
...
EOF
    ...
    echo -e "  Apache Configuration: Will be created for /mrtg access"
    echo ""
    ...
    if ! configure_snmp; then
        ...
        if [[ "${OS_ID}" == "debian" ]] || [[ "${OS_ID}" == "ubuntu" ]]; then
            apt-get install --reinstall -y snmpd >/dev/null 2>&1 || true
        fi
        configure_snmp || error_exit "Failed to configure SNMP after repair attempt"
    fi
...
# =============================================================================
# UNINSTALL FUNCTIONS – FIXED: Handle unbound variables by sourcing install.conf
# =============================================================================
uninstall_mrtg() {
    ...
    if [[ -f "${MRTG_CONF}/install.conf" ]]; then
        ...
        log "INFO" "Loaded installation configuration"
    else
        # Set defaults for uninstall if config not found
