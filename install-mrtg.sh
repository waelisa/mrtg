#!/bin/bash
#
# MRTG Professional Monitoring Suite - Enterprise Edition v2.2.4
...
# with error handling for setfacl
# =============================================================================
create_directories() {
    ...
    if [[ "${PANEL_TYPE}" == "directadmin" ]] && id diradmin >/dev/null 2>&1; then
        ...
        if command -v setfacl >/dev/null 2>&1; then
            setfacl -R -m u:diradmin:rx "${WEB_MRTG_DIR}" 2>/dev/null || log "DEBUG" "setfacl not available, skipping"
...
EOF
    ...
    if [[ -d "${WEB_MRTG_DIR}" ]] && command -v setfacl >/dev/null 2>&1; then
        ...
        log "DEBUG" "setfacl not available, skipping ACL setup"
    fi
...
#!/bin/bash
# Rspamd Statistics Collector for MRTG v2.2.4
...
#!/bin/bash
# MRTG Runner - Generated by MRTG Professional Suite v2.2.4
...
# =============================================================================
# UNINSTALL FUNCTIONS – FIXED: Added proper argument handling
# =============================================================================
